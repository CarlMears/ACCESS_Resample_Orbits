module maps
	...
	integer(4),parameter				:: CELL_CENTERED = 1
	integer(4),parameter				:: EDGE_CENTERED = 2

	integer(4),parameter				:: SPHERICAL     = 1
	integer(4),parameter				:: RECTANGULAR   = 2
	integer(4),parameter				:: CYLINDRICAL   = 3 ! not implemented yet

	real(4)								:: SMALL_NUMBER  = 1.0e-3
...
contains
	...
	subroutine set_up_map(min_x,delta_x,num_x,type_x,min_y,delta_y,num_y,type_y,type_bc,mp,error)

		use NAN_support

		implicit none

		real(4),intent(IN)			:: min_x
		real(4),intent(IN)			:: delta_x
		integer(4),intent(IN)		:: num_x
		integer(4),intent(IN)		:: type_x
		real(4),intent(IN)			:: min_y
		real(4),intent(IN)			:: delta_y
		integer(4),intent(IN)		:: num_y 
		integer(4),intent(IN)		:: type_y
		integer(4),intent(IN)		:: type_bc
		type(map_str),intent(INOUT)	:: mp
		integer(4),intent(OUT)		:: error

		if (finite(min_x)) then
			mp%min_x = min_x
		else
			mp%min_x = NAN
			error = 1
		...
		if ((finite(delta_x)) .and.(delta_x .gt. 0.0)) then
			mp%delta_x = delta_x
		else
			mp%delta_x = NAN
			error = 1
		...
		if ((finite(delta_y)) .and.(delta_y .gt. 0.0)) then
			mp%delta_y = delta_y
		else
			mp%delta_y = NAN
			error = 1
	...
	end subroutine set_up_map

	subroutine zero_map(mp,error)
	...
	end subroutine zero_map




	subroutine deallocate_map(mp,error)
	...
	subroutine get_map_value(mp,xx,yy,value,error)
		...
		type(map_str),intent(IN)		:: mp       ! map structure to be interpolated
		real(4),intent(IN)				:: xx	    ! longitude
		real(4),intent(IN)				:: yy		! latitude
		real(4),intent(OUT)				:: value	! the interpolated value
		integer(4),intent(OUT)			:: error

		integer(4)						:: ix_left,ix_right		! map indices of points to the right (left) of desired point
		integer(4)						:: iy_lower,iy_upper    ! map indices of points above (below) the desired point
		real(4)							:: dx,dy				! distance of the desired point to the
																! right (above) the lower left grid point

		real(4)							:: wt_left,wt_right
		real(4)							:: wt_lower,wt_upper
		real(4)							:: tot,tot_wt,wt
		real(4)							:: x,y,v
		...
		if ((.not. finite(xx)) .or. (.not. finite(yy)) .or. &
		...
		v = map_value(mp,ix_left,iy_lower,error) 
		if (finite(v)) then
			wt = wt_left * wt_lower
			tot = tot +( v * wt)
			tot_wt = tot_wt+wt
		...
		v = map_value(mp,ix_left,iy_upper,error) 
		if (finite(v)) then
			wt = wt_left * wt_upper
			tot = tot + (v * wt)
			tot_wt = tot_wt+wt
		...
		v = map_value(mp,ix_right,iy_upper,error) 
		if (finite(v)) then
		...
		v = map_value(mp,ix_right,iy_lower,error) 
		if (finite(v)) then
	...
	end subroutine get_map_value

	subroutine get_map_value_no_interp(mp,xx,yy,value,error)
		...
		integer(4)						:: ix					! map index of point closest to the desired point
		integer(4)						:: iy					! map index of point closest to the desired point
		...
		value = map_value(mp,ix,iy,error)
		 
		return

	end subroutine get_map_value_no_interp


	subroutine wrap_indices(mp,ix,iy,ix_wrapped,iy_wrapped,error)

		implicit none

		type(map_str),intent(IN)		:: mp
		integer(4),intent(IN)			:: ix
		integer(4),intent(IN)			:: iy
		integer(4),intent(OUT)			:: ix_wrapped
		integer(4),intent(OUT)			:: iy_wrapped
		integer(4),intent(OUT)			:: error
	...
	end subroutine wrap_indices


	real(4)  function map_value(mp,ix,iy,error)
		...
		integer(4)				:: ix_wrapped
		integer(4)				:: iy_wrapped
		...
		if (error .eq. 0) then
			map_value = mp%dat(ix_wrapped,iy_wrapped)
		else
			map_value = NAN
		endif
	...
	subroutine get_grid_location(mp,ix,iy,x,y,error)
		...
		type(map_str),intent(IN)			:: mp		! map structure
		integer(4),intent(IN)				:: ix		! x grid position
		integer(4),intent(IN)				:: iy	    ! y grid position
		real(4),intent(OUT)					:: x        ! x position in real space
		real(4),intent(OUT)					:: y		! y position in real space
	...
	subroutine get_vertex_location(mp,ix,iy,x,y,error)
	...
	end subroutine get_vertex_location



	subroutine get_grid_indices(mp,x,y,ix,iy,error)
	...
	subroutine get_vertex_indices(mp,x,y,ix,iy,error)
	...
	end subroutine get_vertex_indices

	subroutine calc_relative_position(mp,x,y,ix,iy,x_rel,y_rel,error)
		...
		integer(4),intent(IN)				:: ix
		integer(4),intent(IN)				:: iy
		real(4),intent(OUT)					:: x_rel
		real(4),intent(OUT)					:: y_rel
		integer(4),intent(OUT)				:: error
		...
		call get_vertex_location(mp,ix,iy,xv,yv,error)
	...
	end subroutine calc_relative_position



	subroutine downsample_map(oldmap,num_x,type_x,num_y,type_y,newmap,error)

		! this subroutine downsamples a map object to a lower resolution

		type(map_str),intent(IN)		::	oldmap	! original map to be downsampled
		integer(4),intent(IN)			::	num_x	! number of x grid points in new map
		integer(4),intent(IN)			::  type_x	! x grid type for new map
		integer(4),intent(IN)			::	num_y	! number of y grid points in new map
		integer(4),intent(IN)			::  type_y	! y grid type for new map
		type(map_str),intent(OUT)		::  newmap	! new map structure, resampled to new
													! lower resolution
		integer(4),intent(OUT)			::  error   ! error flag
		...
		integer(4)						::  ix_wrapped
		integer(4)						::  iy_wrapped
		real(4),dimension(2,2)			::	box_wts
		real(4)							::  v

		real(4),dimension(:,:),pointer	::  map_sum
		real(4),dimension(:,:),pointer	::	map_sum_wts
		...
		call set_up_map(oldmap%min_x,new_dx,num_x,type_x, &
		...
		do ix = 1,oldmap%num_x
			do iy = 1,oldmap%num_y
				...
				call get_grid_location(oldmap,ix,iy,x,y,error)
				
				! find which vertex in the new map the center of the old cell falls nearest

				call get_vertex_indices(newmap,x,y,ix_new,iy_new,error)

				! find position relative to vertex

				call calc_relative_position(newmap,x,y,ix_new,iy_new,x_rel,y_rel,error)

				! calculate area overlap of the old cell with the four quadrants centered at the
				! vertex.

				call calc_box_weights(x_rel,y_rel,oldmap%delta_x,oldmap%delta_y,box_wts)
			...
			!	endif
				...
				do ix_bx = 0,1
					do iy_bx = 0,1
						v = map_value(oldmap,ix,iy,error)
						if (finite(v)) then
							
							call wrap_indices(newmap,ix_new+ix_bx,iy_new+iy_bx,ix_wrapped,iy_wrapped,error)
...
333							format(1x,2f9.2,i4,i4,2f9.2)
	...
	end subroutine downsample_map 
	
	
	subroutine calc_box_weights(x,y,len_x,len_y,box_wts)

		implicit none

		real(4),intent(IN)					:: x
		real(4),intent(IN)					:: y
		real(4),intent(IN)					:: len_x
		real(4),intent(IN)					:: len_y
		real(4),dimension(2,2),intent(OUT)	:: box_wts
		...
		return
	end subroutine calc_box_weights


	subroutine write_map(mp,unit_num,error)

		implicit none

		type(map_str),intent(IN)			:: mp
		integer(4),intent(IN)				:: unit_num
		integer(4),intent(OUT)				:: error
	...
	subroutine read_map(unit_num,mp,error)

		implicit none

		type(map_str),intent(OUT)			:: mp
		integer(4),intent(IN)				:: unit_num
		integer(4),intent(OUT)				:: error
		...
		call set_up_map(min_x,delta_x,num_x,type_x,min_y,delta_y,num_y,type_y,SPHERICAL,mp,error)
