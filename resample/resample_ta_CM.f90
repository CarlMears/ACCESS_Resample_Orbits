...
module resample
    ...
    type ResampleWeights
        ...
        integer(int32) :: num_output_cells ! = (1+num_extra_fovs)*maxcel - num_extra_fovs 
    ...
    private
    public init_resample_weights,resample_ta_extra
    public resample_89_ta_extra
    public free_resample_weights
...
contains
    ...
    subroutine init_resample_weights(sat_name,bnd_number,target_size,weights)
        ...
        if (sat_name == 'AMSR2') then
            if (bnd_number .le. 5) then
                weights%num_input_cells  = maxcel 
                weights%num_input_scans  = 29
            else
                weights%num_input_cells  = maxcel_89
                weights%num_input_scans  = 57
            endif
        else if (sat_name == 'SSMI') then
            weights%num_input_cells  = maxcel
            weights%num_input_scans  = 29
        else
        ...
        weights%num_output_cells = (1+num_extra_fovs)*maxcel - num_extra_fovs 
        ...
        select case (sat_name)
            ...
            case('SSMI')
                ksat = 13
                write(weight_file_name,101)trim(wt_path),trim(sat_name),ksat,target_size,bnd_number-1  !band number in files names 1 less
    ...
    subroutine resample_ta_extra(weights,ichan)
        ...
        integer(int32) :: iscan,kscan,kscan_start,kscan_end,kfov
        integer(int32) :: scan_number_out
        real(real64)   :: scan_index_out
        integer(int32) :: jfov,jscan
        real(real64)   :: wt_sum,ta_sum
        real(real64)   :: wt_sum2,ta_sum2
        ...
        do kscan = kscan_start,kscan_end
            do kfov = 1,weights%num_output_cells
                ...
                do iscan = 16,maxscan-16  ! this is the index scan
                    scan_number_out = ((iscan-1)*(1+num_extra_scans))+kscan-kscan_start+1
                    scan_index_out  = iscan+kscan*(1.0/(1+num_extra_scans))
                    ...
                    do ipos = 1,num_select
                        ...
                        if ((ta(jfov,iscan+jscan,ichan) .gt. 0.01) .and. (ta(jfov,iscan+jscan,ichan) .lt. 400.0)) then
                            wt_sum = wt_sum + jwt
                            ta_sum = ta_sum + jwt*ta(jfov,iscan+jscan,ichan)
                        endif
                    enddo
                    !print *,scan_number_out,kfov,wt_sum,ta_sum,num_cells_included
                    if (wt_sum > 0.95) then
                        ta_extra(kfov,scan_number_out,ichan) = ta_sum/wt_sum
                    else
                        ta_extra(kfov,scan_number_out,ichan) = nan_f32
                    endif
        ...
        enddo !kscan
        deallocate(wt_slice)
    end subroutine resample_ta_extra

    subroutine resample_89_ta_extra(weights,ichan)
        ...
        allocate(wt_slice(weights%num_input_cells,weights%num_input_scans))
        !The outer loops are the position in the output scan....
        do kscan = kscan_start,kscan_end
            do kfov = 1,weights%num_output_cells
                wt_slice = weights%wts(:,:,kfov,kscan-kscan_start+1)
                !Make a list of the non-zero elements
                ...
                do jscan = -28,28
                    do jfov = 1,maxcel_89
                        if (abs(wt_slice(jfov,jscan+29)) > 0.00000001) then
                            num_select= num_select+1
                            jscan_list(num_select) = jscan
                            jfov_list(num_select) = jfov
                            wt_list(num_select) = wt_slice(jfov,jscan+29)
                        endif
                    enddo
                enddo

                !for now, loop over all the positions
                do iscan = 30,maxscan_89-30  ! this is the index scan
                    scan_number_out = iscan
                    wt_sum = 0.0D0
                    ta_sum = 0.0D0
                    num_cells_included = 0
                    ...
                    do ipos = 1,num_select
                        ...
                        jwt = wt_list(ipos)
                        num_cells_included = num_cells_included + 1
                        if ((ta_89(jfov,iscan+jscan,ichan-12) .gt. 0.01) .and. (ta_89(jfov,iscan+jscan,ichan-12) .lt. 400.0)) then
                            wt_sum = wt_sum + jwt
                            ta_sum = ta_sum + jwt*ta_89(jfov,iscan+jscan,ichan-12)
                        endif
                    enddo
                    !print *,scan_number_out,kfov,wt_sum,ta_sum,num_cells_included
                    if (wt_sum > 0.95) then
        ...
        enddo !kscan
        deallocate(wt_slice)
    end subroutine resample_89_ta_extra
        ...
        do jscan = 1,maxscan_extra
            if (ieee_is_finite(scan_time_extra(jscan))) then
                do jfov = 1,maxcel_extra
                    if (ta_extra(jfov,jscan,channel) > 1.0) then
    ...
    subroutine resample_onto_lat_lon_grid_interp(resolution,channel,do_time,gridded_time,gridded_tbs)
            ...
            do jfov = 1,maxcel_extra-1
                ...
                do kscan=jscan,jscan+1
                    do kfov=jfov,jfov+1
                        ...
                        if (.not. ieee_is_finite(ta_extra(kfov,kscan,channel))) bad_loc = .true.
                        if (ta_extra(kfov,kscan,channel) < 20.0) bad_loc = .true.
                        if (ta_extra(kfov,kscan,channel) > 350.0) bad_loc = .true.
                    enddo
                enddo
                ...
                do kscan=jscan,jscan+1
                    do kfov=jfov,jfov+1
                        ...
                        quad_ta(iquad) = ta_extra(kfov,kscan,channel)
                ...
                do ilatx = klat-1,klat+1
                    do ilonx = klon-1,klon+1
                        ...
                        if (distance < gridded_distance%dat(ilon,ilat)) then
                            if ((ieee_is_finite(ta_extra(jfov,jscan,channel))) .and. &
                                (ta_extra(jfov,jscan,channel) < 350.0) .and. &
                                (ta_extra(jfov,jscan,channel) > 0.001)) then
                                    ilon_closest = ilon
                                    ilat_closest = ilat

                                ! replace values at this location
                                gridded_tbs%dat(ilon,ilat) = ta_extra(jfov,  &
                                                                     jscan, &
                                                                     channel)
    ...
    subroutine resample_time_onto_lat_lon_grid(resolution,gridded_time,gridded_distance)
