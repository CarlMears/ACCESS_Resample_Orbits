...
contains
    ...
    real(4) function distance_between_lon_lats(lon1,lat1,lon2,lat2,error)
        ...
        !use trig_degrees, only: sind, cosd
            ...
            error = 0
            if((lon1 .lt. 0.0) .or. (lon1 .gt. 360.0) .or. &
               (lon2 .lt. 0.0) .or. (lon2.gt. 360.0) .or. &
               (lat1 .lt. -90.0) .or. (lat1 .gt. 90.0) .or. &
               (lat2 .lt. -90.0) .or. (lat2 .gt. 90.0) .or. &
               (.not. (ieee_is_finite(lon1))) .or. &
               (.not. (ieee_is_finite(lon2))) .or. &
            ...
            distance_64 = distance_between_lon_lats_real64(real(lon1,kind=real64), &
                                                                         real(lat1,kind=real64), &
                                                                         real(lon2,kind=real64), &
